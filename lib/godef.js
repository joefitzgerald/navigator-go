...
class Godef {
  ...
  gotoDefinitionForWordAtCursor () {
    ...
    if (editor.hasMultipleCursors()) {
      atom.notifications.addWarning('navigator-godef', {
        dismissable: true,
        icon: 'location',
        description: 'godef only works with a single cursor'
      })
      return Promise.resolve(false)
    }
  ...
  gotoDefinitionWithParameters (cmdArgs, cmdInput = undefined) {
    ...
    return this.checkForTool().then((cmd) => {
      if (!cmd) {
        return
      }
  ...
  parseGodefLocation (godefStdout) {
    ...
    let colNumber = outputs.pop()
    let rowNumber = outputs.pop()
    ...
    if (!loc || !loc.filepath) {
      if (loc) {
        atom.notifications.addWarning('navigator-godef', {
          dismissable: true,
          icon: 'location',
          description: 'godef returned malformed output:',
          detail: JSON.stringify(loc.raw)
        })
      } else {
        atom.notifications.addWarning('navigator-godef', {
          dismissable: true,
    ...
    return fs.stat(loc.filepath, (err, stats) => {
      if (err) {
        ...
        atom.notifications.addWarning('navigator-godef', {
          dismissable: true,
          icon: 'location',
          description: 'godef returned invalid file path:',
          detail: loc.filepath
        })
        return false
      }
  ...
  visitFile (loc, callback) {
    return atom.workspace.open(loc.filepath).then((editor) => {
      if (loc.pos) {
        ...
        this.cursorOnChangeSubscription = this.highlightWordAtCursor(atom.workspace.getActiveTextEditor())
      }
    })
  }
  ...
  visitDirectory (loc, callback) {
    return this.findFirstGoFile(loc.filepath).then((file) => {
      ...
      atom.notifications.addWarning('navigator-godef', {
        dismissable: true,
        icon: 'location',
        description: 'godef return invalid directory:',
        detail: loc.filepath
      })
    })
  }
